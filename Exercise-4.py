...
dataDF: DataFrame = spark.createDataFrame(dataRDD, myManualSchema)
...
testEvaluator: RegressionEvaluator = RegressionEvaluator(labelCol="label", metricName="rmse", predictionCol="prediction")
...
# Create a DataFrame with the given x values
newData = [(x,) for x in [-1.5, 2.5, 8.5, 31.4]]   # single-element tuple 
newDF: DataFrame = spark.createDataFrame(newData, ["X"])
assembledNewDF: DataFrame = vectorAssembler.transform(newDF)
newPredictions: DataFrame = lrModel.transform(assembledNewDF)
...
numberOfCharacters: int = articlesRdd.flatMap(lambda line: list(line)).filter(lambda char: char != ' ').count()
...
distinctWordCount=articlesRdd.flatMap(lambda line: line.split()).filter(lambda word: len(word) == 6).countByValue()
commonWord: str = max(distinctWordCount, key=distinctWordCount.get)
commonWordCount: int = distinctWordCount[commonWord]
