...
retailerDataFrame: DataFrame = spark \
                              .read \
...
best12DaysDF: DataFrame = retailerDataFrame.withColumn("sales", functions.col("QUANTITYORDERED") * functions.col("PRICEEACH")).\
    groupBy("ORDERDATE").\
    sum("sales").\
    withColumnRenamed("sum(sales)","total_sales").\
    sort(functions.desc("total_sales")).\
    limit(12)
...
productDF: DataFrame = retailerDataFrame.withColumn("sales", functions.col("QUANTITYORDERED") * functions.col("PRICEEACH")).\
    filter(functions.col("YEAR_ID") == 2003).\
    withColumn("sales", functions.when(functions.col("MONTH_ID") == 1, functions.col("sales") * 2).otherwise(functions.col("sales"))).\
    groupBy("PRODUCTCODE").\
    sum("sales").\
    withColumnRenamed("sum(sales)","total_sales").\
    sort(functions.desc("total_sales")).\
...
sales2017Row: Row = salesDS.filter(functions.col("year") == 2017).first()
sales2017: Sales = Sales(**sales2017Row.asDict())
print(f"Sales for 2017 is {sales2017.euros}")

maximumSalesRow: Row = salesDS.sort(functions.desc("euros")).first()          # if use agg(max()), need join
maximumSales: Sales = Sales(**maximumSalesRow.asDict())
...
multiSales2016Row: Row = multiSalesDS.groupBy("year").sum("euros").\
                                 withColumnRenamed("sum(euros)","euros").\
                                 filter(functions.col("year") == 2016).first()

multiSales2016: Sales = Sales(**multiSales2016Row.asDict())

print(f"Total sales for 2016 is {multiSales2016.euros}")

"""maximumMultiSalesRow: Row = multiSalesDS.groupBy("year").sum("euros").\
                                 withColumnRenamed("sum(euros)","euros").\
                                 sort(functions.desc("euros")).\
                                 first()
maximumMultiSales: Sales = Sales(**maximumMultiSalesRow.asDict())
"""  # takes 5 secs

multiSalesDS.createOrReplaceTempView("multiSalesTable")
maximumMultiSalesRow = spark.sql("""
    SELECT year, SUM(euros) AS euros
    FROM multiSalesTable
    GROUP BY year
    ORDER BY euros DESC
    LIMIT 1
""").first()
maximumMultiSales = Sales(**maximumMultiSalesRow.asDict())
...
my_ex3_folder: str = "retailer_data_ex3"
...
retailerStreamingDF: DataFrame = spark.readStream.schema(myManualSchema)\
                                        .option("header","true") \
                                        .option("delimiter", ";") \
                                        .csv(targetFiles)
...
bestDaysDFStreaming: DataFrame = retailerStreamingDF.withColumn("sales", functions.col("QUANTITYORDERED") * functions.col("PRICEEACH")).\
    groupBy("ORDERDATE").\
    sum("sales").\
    withColumnRenamed("sum(sales)","total_sales").\
    sort(functions.desc("total_sales"))
...
# MAGIC
# MAGIC 1. How well did the streaming data example work for you?
# MAGIC
# MAGIC Initially, the streaming data process encountered some issues, producing results different from the expected output. However, I tried to rerun it another time, it successfully generated the correct output.
# MAGIC
# MAGIC 2. What was the final output for the streaming data for you?
# MAGIC
# MAGIC The final output was presented in a tabular format, closely resembling the output structure from the static data in Task 2. 
# MAGIC
# MAGIC The data in the streaming tasks is the same as the earlier static data (just divided into multiple files).
# MAGIC 3. Did the final output match the first six rows of the task 2 output?
# MAGIC
# MAGIC Yes, the final output precisely matched the first six rows of the Task 2 static data output.
# MAGIC
# MAGIC 4. If it did not, what do you think could be the reason?
# MAGIC
# MAGIC Initially, on my first run, there were discrepancies. It can be attributed to environmental factors such as high usage volumes in the Databricks environment. However, rerunning the process successfully yielded the correct results.
# MAGIC
# MAGIC 5. If you had problems, what were they?
# MAGIC
# MAGIC When I attempted to run the latter cell independently of the first cell or vice versa, first cell running continuously without halting, which needed manual cancellation.
# MAGIC
# MAGIC 6. And what do you think were the causes for those problems?
# MAGIC
# MAGIC The issues stemmed from my misunderstanding of the instructions and the nature of streaming data.
