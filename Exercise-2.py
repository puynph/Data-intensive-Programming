...
from pyspark.sql.functions import year, min, max, desc, count, avg, col, round
...
weatherDataFrame: DataFrame = spark \
                              .read \
...
result = spark.sql("""
          SELECT MIN(temperaturemin) AS minTemp, MAX(temperaturemax) AS maxTemp
          FROM weather_data
          """)

# Collect the results as a list of rows
min_max_temperatures = result.collect()

minTemp: float = min_max_temperatures[0].minTemp
maxTemp: float = min_max_temperatures[0].maxTemp
...
aggregatedDF: DataFrame = weatherDataFrameWithYear.groupBy("year"). \
        agg(min("temperaturemin").alias("min_temperature"), \
...
task6DF: DataFrame = (weatherDataFrameWithYear.groupBy("year")). \
        agg(min("temperaturemin").alias("min_temperature"), \
            max("temperaturemax").alias("max_temperature"), \
            count("*").alias("entries"), \
...
min2012: float = task6DF.filter(task6DF.year == 2012).select("min_temperature").collect()[0][0]
max2016: float = task6DF.filter(task6DF.year == 2016).select("max_temperature").collect()[0][0]
entries2018: int = task6DF.filter(task6DF.year == 2018).select("entries").collect()[0][0]
wind2008: float = task6DF.filter(task6DF.year == 2008).select("avg_windspeed").collect()[0][0]
...
yearsWithFog = weatherDataFrameWithYear.filter(weatherDataFrameWithYear.fog == 'Yes')

yearWithMostDaysWithFog: DataFrame = yearsWithFog.groupBy('year'). \
                                agg(count('*').alias("foggy_days")). \
                                sort(desc("foggy_days")).first()
                                
yearWithMostDaysWithFog: int = yearWithMostDaysWithFog.year
