...
df_csv = spark.read.option("inferSchema","true").option("header","true").option("delimiter", "|").csv(source_csv_folder)
display(df_csv)
...
df_csv.write.option("inferSchema","true").option("header","true").option("delimiter", "|").csv(target_file_csv)
...
df_new_rows = df_csv.filter(functions.col("City") == "New York").sort(functions.desc("Start_Time")).limit(more_rows_count)

# Wrong implementation: withColumn("ID", functions.col("ID") + "_Z1")
...
df_new_rows.write.option("inferSchema","true").option("header","true").option("delimiter", "|").mode("append").csv(target_file_csv)
...
df_new_csv = spark.read.option("inferSchema","true").option("header","true").option("delimiter", "|").csv(target_file_csv)
...
df_modified = df_new_rows.withColumn("ID", functions.regexp_replace(functions.col("ID"), "_Z1", "_Z2")) \
    .withColumn("AddedColumn1", functions.concat(functions.lit("prefix-"), functions.col("City"))) \
    .withColumn("AddedColumn2", functions.lit("New column")) \
    .drop("Description")

# Display the new modified rows
df_modified.show(2)

# Append the new modified rows to CSV storage
df_modified.write.option("inferSchema","true").option("header","true").option("delimiter", "|").mode("append").csv(target_file_csv)
...
modified_csv_df = spark.read.option("inferSchema","true").option("header","true").option("delimiter", "|").csv(target_file_csv)
...
# MAGIC
# MAGIC There are some dissimilarities in the output: the values in 'Start_Time' and 'End_Time' columns, some values in column 'Temperature_F' has prefix-City.
# MAGIC
# MAGIC *What is your explanation/guess for why the CSV seems broken and the schema cannot be inferred anymore?*
# MAGIC
# MAGIC It might be because the df_modified with different schema was appended to the target csv file, this led to inconsistencies in data frame's schema, resulting in the seemingly broken CSV 
# MAGIC
...
# MAGIC
# MAGIC My output is similar to the second alternative, and i's not similar to the data in Parquet storage: 2 new columns 'AddedColumn1' and 'AddedColumn2' are added, 'Description' is dropped and new values with modified format are added to ID column.
# MAGIC
...
# MAGIC
# MAGIC I think that when appending data with a different schema to an existing Parquet file, it might lead to issues like dropping columns or modifying the existing schema. Therefore, the output only consists of 9 columns as the newly modified schema
# MAGIC
...
(deltatable.alias("target")
        .merge(df_delta_update.alias("source"), condition="target.ID = source.ID")
        .whenMatchedUpdate(set = {"target.Temperature_F" : "source.Temperature_F"})
